...
def build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--pretrained", type=str, default="true", choices=["true", "false"], help="use pretrained weights")
    ...
    p.add_argument("--debug-token-stats", action="store_true", help="print per-block debug logs")

    # Ours-specific knobs
    p.add_argument("--selector", type=str, default="hquota_ff", help="selector name")
    p.add_argument("--hq-q", type=float, default=0.30, help="head-quota fraction [0,1]")
    p.add_argument("--cand-extra", type=int, default=16, help="extra candidate pool size")
    p.add_argument("--merges", type=str, default="v", choices=["v", "kv"], help="merge mode")
    p.add_argument("--alpha", type=float, default=0.15, help="merge alpha")
    ...
    p.add_argument("--select-mode", type=str, default="keep", choices=["keep", "drop"], help='selector mode: "keep" keeps K=T-r (default), "drop" removes r')
...
def find_transformer_blocks(model: nn.Module):
...
def build_eval_loader(args: argparse.Namespace, model: nn.Module) -> DataLoader:
    ...
    pin_mem = (args.pin_memory.lower() == "true") if hasattr(args, "pin_memory") else True
    loader = DataLoader(dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.workers, pin_memory=pin_mem)
    return loader


@torch.no_grad()
def run_inference(model: nn.Module, args: argparse.Namespace) -> Tuple[float, float]:
...
def main():
    ...
    # Print meter summary (ours only)
    meter = getattr(model, "_ours_meter", None)
    if meter is not None:
        print("\n[Measure] Per-block token/time summary:")
        print(meter.summary())
